...
export default function Dashboard() {
  ...
  /**
   * Reset to the empty state. Wordmark click handler — within the dashboard,
   * does NOT bring the splash back. Clears results, hash, sessionStorage form
   * preload so the form returns to its Mag 7 default.
   */
  function handleHomeReset() {
    ...
    formRef.current?.applyRequest(buildPreset('mag7'));
  }

  return (
    <div className="dashboard-enter flex min-h-screen flex-col bg-surface text-ink">
      <header className="flex items-center justify-between border-b border-rule px-5 py-4 sm:px-8">
        <button
          type="button"
          onClick={handleHomeReset}
          aria-label="Strata — return to start"
          className="font-serif text-2xl tracking-tight text-ink transition-colors hover:text-accent"
